import { Mesh, Bone, Vector3, Color, BufferGeometry, Float32BufferAttribute, BufferAttribute, MeshNormalMaterial, DoubleSide } from 'three'
...
const loadCharacter = async (filename: string): Promise<Mesh[]> => {
    const req = await fetch(`/MegaManLegends2-VRM/${filename}`);
    if (!req.ok) {
        // Handle HTTP errors
        throw new Error(`Failed to load ${filename}: ${req.status} ${req.statusText}`);
    }
    ...
    const bodyMesh = bodyStrips.map(strip => {
        ...
        triangleFaces.forEach(face => indices.push(face[0].index, face[1].index, face[2].index));
        quadFaces.forEach(face => indices.push(face[0].index, face[1].index, face[2].index));
